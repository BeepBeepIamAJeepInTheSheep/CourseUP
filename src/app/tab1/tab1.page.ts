...
export class Tab1Page implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Initialize wishList from the service
    this.wishList = this.wishListService.getWishList();
  }

  addToCart(courseData: any): void {
    this.wishListService.addToCart(courseData);
    this.glowingItems = courseData;
    setTimeout(() => this.glowingItems = null, 250);
  }

  toggleWishList(courseData: any): void {
    if (this.isInWishList(courseData)) {
      this.removeFromWishList(courseData);
    } else {
      this.addToWishList(courseData);
    }
  }

  addToWishList(courseData: any): void {
    this.wishListService.addToWishList(courseData);
  }

  removeFromWishList(courseData: any): void {
    this.wishListService.removeFromWishList(courseData);
  }

  isInWishList(courseData: any): boolean {
    return this.wishListService.isInWishList(courseData);
  }
  ...
  // toggleGlowEffect(item: any): void {
  //   if (this.glowingItems.has(item)) {
  //     this.glowingItems.delete(item);
  //   } else {
  //     this.glowingItems.add(item);
  //     setTimeout(() => this.glowingItems.delete(item), 200);
  //   }
  // }

  isGlowing(courseData: any): boolean {
    return this.glowingItems === courseData;
  }
